...
/*__JARVIS_TESTABLE_RENDER_START__*/
function fmtWhen(value){{if(!value)return 'never';try{{return new Date(value).toLocaleString();}}catch(e){{return value;}}}}
function shortLog(value){{if(!value)return 'No log line found';const s=String(value);return s.length>180?s.slice(0,179)+'...':s;}}
function botPillClass(bot){{if(bot.state==='error'||bot.state==='check')return 'down';return 'up';}}
function jarvisMetaCell(label,value){{const span=document.createElement('span');span.appendChild(document.createTextNode(label));const strong=document.createElement('strong');strong.textContent=(value==null?'':String(value));span.appendChild(strong);return span;}}
function renderSearchResults(listEl,results){{listEl.replaceChildren();(results||[]).forEach(item=>{{const li=document.createElement('li');const strong=document.createElement('strong');strong.textContent=(item.collection==null?'':String(item.collection));li.appendChild(strong);li.appendChild(document.createTextNode(item.snippet==null?'':String(item.snippet)));listEl.appendChild(li);}});}}
function renderSocialBots(container,bots){{container.replaceChildren();(bots||[]).forEach(bot=>{{const art=document.createElement('article');art.className='socialBot';const top=document.createElement('div');top.className='socialBotTop';const h3=document.createElement('h3');h3.textContent=(bot.title==null?'':String(bot.title));top.appendChild(h3);const pill=document.createElement('span');pill.className='pill '+botPillClass(bot);pill.textContent=(bot.badge_label||'Check');top.appendChild(pill);art.appendChild(top);const desc=document.createElement('p');desc.textContent=(bot.description==null?'':String(bot.description));art.appendChild(desc);const meta=document.createElement('div');meta.className='socialBotMeta';meta.appendChild(jarvisMetaCell('Schedule',bot.schedule_state||'none'));meta.appendChild(jarvisMetaCell('Service',bot.service_state||'none'));meta.appendChild(jarvisMetaCell('Last run',fmtWhen(bot.last_run_at)));meta.appendChild(jarvisMetaCell('Last posted',fmtWhen(bot.last_posted_at)));meta.appendChild(jarvisMetaCell('Posted total',bot.posted_count));meta.appendChild(jarvisMetaCell('24h errors',bot.errors_24h));art.appendChild(meta);const logDiv=document.createElement('div');logDiv.className='socialBotLog';logDiv.textContent=shortLog(bot.latest_log_line);art.appendChild(logDiv);container.appendChild(art);}});}}
/*__JARVIS_TESTABLE_RENDER_END__*/
async function refreshSocialBots(){{const panel=document.getElementById('socialBotsList');const summary=document.getElementById('socialBotsSummary');if(!panel||!summary)return;try{{const r=await fetch('/api/social-bots',{{cache:'no-store'}});const p=await r.json();const attention=p.summary.needs_attention??p.summary.needs_check??0;summary.textContent=`${{p.summary.alive}} alive · ${{attention}} need attention · ${{p.summary.posted_total}} posted`;renderSocialBots(panel,p.bots);}}catch(e){{summary.textContent='Bot status unavailable';const empty=document.createElement('div');empty.className='socialBotEmpty';empty.textContent='Social bot status unavailable.';panel.replaceChildren(empty);}}}}
